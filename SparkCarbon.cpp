...
void setup() {
 ...
 pinMode(PIR, INPUT);
 pinMode(LDR, INPUT);
 rtc.begin(&UDPClient, "storage.fritz.box");
 rtc.setTimeZone(+1);
 carbon.begin(&tcpClient, { 1, 1, 1, 4 });
 dht.begin();
 bmp.begin();
 looptime=millis();
}
...
void loop() {
   if ((millis() > looptime + loopdelay) && sending==false) {
     ...
     carbon.sendData("sparkcore.3.reed0", reed0, now);
     ...
     carbon.sendData("sparkcore.3.reed1", reed1, now);
     ...
     if (lowcount >= 10) { // report those less often     
       ...
       if (dhttemperature==DHTNAN || dhthumidity==DHTNAN) {
           ...
           carbon.sendData("sparkcore.3.humidity", humidity, now);
           sprintf(temperature,"%.1f",dhttemperature);   
           carbon.sendData("sparkcore.3.temperature", temperature, now);
       }
       lowcount=0;
     }
